...
sprite_t init_sprite_ammo(game_t *game, float tx, float ty)
{
    sprite_t sprite = {0};
...
static sprite_t calcul_sprite_ammo(game_t *game, ammo_drop_t *ammo)
{
    sprite_t sprite = {0};
    ...
    float sprite_x = ammo->position.x - game->player->x;
    float sprite_y = ammo->position.y - game->player->y;
    ...
    if (sprite_x <= 10 && sprite_x >= 0 && game->i == true
        ...
        --game->player->hp;
    sprite = init_sprite_ammo(game, tx, ty);
...
void render_sprite_ammo(game_t *game, ammo_drop_t *ammo, sprite_t *spr)
{
    float tex_height = sfTexture_getSize(ammo->texture).y;
    ...
    spr->states = (sfRenderStates) {.texture = ammo->texture,
...
{
    sprite_t spr = calcul_sprite_ammo(game, ammo);
